...
[ ] **Step 4: Shared CSS & Design Tokens**: Implement a premium, dark-mode-first CSS design system in `src/client/index.css` with glassmorphic styles and animations.
...
[ ] **Step 6: Mobile Scanner & JIT Modal**: Implement the `@zxing/library` camera barcode scanner with a toggle for "Add/Subtract" mode and a modal to enter a name for unregistered barcodes.
[ ] **Step 7: PC Dashboard & Reports**: Create the desktop view displaying the inventory grid (search, edit, delete), activity logs feed, and custom SVG charts showing inventory change trends.
