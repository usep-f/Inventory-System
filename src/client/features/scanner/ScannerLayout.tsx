...
export default function ScannerLayout() {
  ...
  // Get most recent scans (up to 10) locally so UI feels fast, or just rely on 'logs' fetched from backend
  const recentLogs = useMemo(() => logs.slice(0, 10), [logs]);

  const handleScan = async (barcode: string) => {
    const result = await scanProduct(barcode, mode);
    
    if (result.success) {
      playBeep('success');
    } else if (result.error === 'NOT_FOUND') {
      playBeep('error');
      // Pause camera and open JIT Modal
  ...
  const onJitSuccess = async () => {
    // Re-scan it immediately now that it's added!
    if (jitBarcode) {
      await scanProduct(jitBarcode, mode);
      playBeep('success');
    }
    closeJitModal();
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100dvh', position: 'relative', overflow: 'hidden', background: '#0a0c14' }}>
      ...
      {/* Top Right Refresh/Switch (placeholder functionality) */}
      ...
      <div style={{ position: 'absolute', bottom: 'calc(44vh - 30px)', left: '50%', transform: 'translateX(-50%)', zIndex: 20 }}>
        <button
          ...
          style={{
            ...
            background: isAdd ? 'var(--success)' : 'var(--error)',
            color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center',
            boxShadow: `0 6px 24px ${isAdd ? 'rgba(1, 181, 116, 0.35)' : 'rgba(238, 93, 80, 0.35)'}`,
      ...
      {/* JIT Modal */}
      {jitBarcode && (
        <JitModal barcode={jitBarcode} onClose={closeJitModal} onSuccess={onJitSuccess} />
      )}
    </div>
  );
}
